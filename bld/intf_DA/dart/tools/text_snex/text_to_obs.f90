...
use      obs_kind_mod, only : SNOW_WATER_EQUIVALENT, SNOW_DEPTH
...
real(r8) :: sdepth, sderr, swe, swerr, qc
...
obsloop: do    ! no end limit - have the loop break when input ends
   ...
   ! assume there is an obs type, where otype=1 is snow depth 
   ! and otype=2 is snow water equivalent
   ...
   if (otype == 1) then
      read(input_line(3:129), *, iostat=rcio) lat, lon, vert, &
                                 ...
                                 sdepth, sderr
      if (rcio /= 0) then 
         if (debug) print *, 'got bad read code getting rest of snow depth obs, rcio = ', rcio
         exit obsloop
      endif
   else
      read(input_line(3:129), *, iostat=rcio) lat, lon, vert, &
                                  ...
                                  swe, swerr
      if (rcio /= 0) then 
         if (debug) print *, 'got bad read code getting rest of swe obs, rcio = ', rcio
         exit obsloop
      endif
   endif
   ...
   ! otype=1 is total snow depth
   ! otype=2 is total SWE
   ...
   if (otype == 1) then
      ...
      call create_3d_obs(lat, lon, vert, VERTISHEIGHT, sdepth, &
            SNOW_DEPTH, sderr, oday, osec, qc, obs)
      call add_obs_to_seq(obs_seq, obs, time_obs, prev_obs, prev_time, first_obs)

      if (debug) print *, 'added snow depth obs to output seq'
   else
      ! make an obs derived type, and then add it to the sequence
      call create_3d_obs(lat, lon, vert, VERTISHEIGHT, swe, &
            SNOW_WATER_EQUIVALENT, swerr, oday, osec, qc, obs)
      call add_obs_to_seq(obs_seq, obs, time_obs, prev_obs, prev_time, first_obs)
      if (debug) print *, 'added snow water equivalent to output seq'
    endif
